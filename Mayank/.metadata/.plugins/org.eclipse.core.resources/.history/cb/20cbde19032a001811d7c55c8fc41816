...
public class VerifyTitle
{
	...
	public void tearDown(ITestResult result, Method method)
	{
		...
		else if(result.getStatus()==ITestResult.SUCCESS)
		{
			screenshot_path=Utility.captureScreenshot(driver, result.getName());
			image=Utility.logger.addScreenCapture(screenshot_path);
			Utility.logger.log(LogStatus.PASS, method.getAnnotation(Test.class).description(), image);
		}
		else
		{
			screenshot_path=Utility.captureScreenshot(driver, result.getName());
			image=Utility.logger.addScreenCapture(screenshot_path);
			Utility.logger.log(LogStatus.SKIP, method.getAnnotation(Test.class).description(), image);
		}
